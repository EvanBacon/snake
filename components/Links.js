...
export default function Links({ show }) {
  return (
    <View style={[styles.linkContainer, { opacity: show ? 0.6 : 0.0 }]} pointerEvents={show ? 'auto' : 'none'}>
      <Text style={styles.link} accessibilityRole="link" href={link} target="_blank" onPress={() => Linking.openURL(link)}>Evan Bacon 🥓</Text>
      <Text style={styles.link} accessibilityRole="link" href={sourceLink} target="_blank" onPress={() => Linking.openURL(sourceLink)}>{`</>`}</Text>
    </View>
  )
}
